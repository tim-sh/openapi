...
} = require('./modules/builders/schemas');
const { ref, responseRef, parameterRef } = require('./modules/utils/references');
        ...
        if (insertRestrictions.Insertable !== false) {
            ...
            pathItem.post = {
                ...
                requestBody: {
                    ...
                    content: {
                        'application/json': {
                            schema: typeRef(element.$Type, SUFFIX.create),
                        }
                    }
                },
        ...
        if (updateRestrictions.Updatable !== false) {
            ...
            const operation = {
                ...
                requestBody: {
                    ...
                    content: {
                        'application/json': {
                            schema: typeRef(element.$Type, SUFFIX.update),
                        }
                    }
                },
    ...
    function entityKey(entityType, level) {
        ...
        keys.forEach((key, index) => {
            ...
            const param = {
                ...
                required: true,
                schema: getSchema(property, '', true, false, { csdl, voc, typesToInline, requiredSchemas, namespace })
            };
            params.push(param);
        })
    ...
    */
    ...
    function pathItemAction(paths, prefix, prefixParameters, actionName, overload, sourceName, actionImport = {}) {
        ...
        if (parameters.length > 0) {
            ...
            parameters.forEach(p => { requestProperties[p.$Name] = getSchema(p, '', false, false, { csdl, voc, typesToInline, requiredSchemas, namespace }) });
    ...
    function pathItemFunction(paths, prefix, prefixParameters, functionName, overload, sourceName, functionImport = {}) {
        ...
        parameters.forEach(p => {
            ...
            if (p.$Collection || p.$Type == 'Edm.Stream'
                ...
                param.schema = getSchema(p, '', true, true, { csdl, voc, typesToInline, requiredSchemas, namespace });
            }
            params.push(param);
        });
    ...
    function response(code, description, type, errors, isCount = true) {
        ...
        if (code != 204) {
            const s = getSchema(type, '', false, false, { csdl, voc, typesToInline, requiredSchemas, namespace });
            r[code].content = {
                'application/json': {}
            };
    ...
    function getSchemas(csdl) {
        ...
        for (const r of requiredSchemas.list) {
            ...
            switch (type.$Kind) {
                case "ComplexType":
                case "EntityType":
                    schemasForStructuredType(unordered, r.namespace, r.name, type, r.suffix, { csdl, voc, typesToInline, requiredSchemas, namespace, derivedTypes });
                    break;
                case "EnumType":
                    schemaForEnumerationType(unordered, r.namespace, r.name, type, { csdl, voc, typesToInline, requiredSchemas, namespace });
                    break;
                case "TypeDefinition":
                    schemaForTypeDefinition(unordered, r.namespace, r.name, type, { csdl, voc, typesToInline, requiredSchemas, namespace });
                    break;
            }
        }
     ...
     * @return {object} Reference Object
     */
    function typeRef(typename, suffix = '') {
        let name = typename;
        let nsp = '';
        let url = '';
